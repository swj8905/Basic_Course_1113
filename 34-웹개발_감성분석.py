...
if button:
    ...
    while True:
        try:
            ...
            if score >= 0.5:
                st.write(f"{score * 100:.2f} % 확률로 **긍정**입니다.   ")
            else:
                st.write(f"{100 - score * 100:.2f} % 확률로 **부정**입니다.   ")
            st.write("---------")
            if 0.8 <= score <= 1:
    ...
    st.write("# 감성분석 결과")
    bar_x = alt.X("감성", sort=list(sentiment_result.keys()))
    fig = alt.Chart(result).mark_bar().encode(x=bar_x, y="댓글 수")
    st.altair_chart(fig, use_container_width=True)

    circle_scale = alt.Scale(domain=[1, df["reply"].max()])
    fig_circle = alt.Chart(df).mark_circle().encode(
        x="score",
        y="like",
    ...
    st.altair_chart(fig_circle, use_container_width=True)
